...
def align_schema(real_data: pd.DataFrame, synth_data: pd.DataFrame, 
    ...
    # For now, just extract target column if specified
    target_column = None
    if spec.target:
...
def standardize_missing_values(df: pd.DataFrame, strategy: str = "pandas") -> pd.DataFrame:
    ...
    if strategy == "pandas":
        # Keep pandas NA representation
        pass
    elif strategy == "zero":
        # Replace NaN with 0 for numeric, "missing" for categorical
...
def _are_compatible_types(dtype1, dtype2) -> bool:
    ...
    # Both numeric
    if (np.issubdtype(np_dtype1, np.number) and 
        np.issubdtype(np_dtype2, np.number)):
        return True
    ...
    # Both datetime
    if (np.issubdtype(np_dtype1, np.datetime64) and 
        np.issubdtype(np_dtype2, np.datetime64)):
        return True
